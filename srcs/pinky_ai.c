...
char	**pinky_move(t_mlx_data *data, t_mlx_map *map)
{
	...
	if (move == UP)
	{
		...
		data->enemy.pinky.stash = map->map[data->enemy.pinky.y - 1][data->enemy.pinky.x];
	...
	if (move == LEFT)
	{
		...
		temp = data->enemy.pinky.stash;
		data->enemy.pinky.stash = map->map[data->enemy.pinky.y][data->enemy.pinky.x - 1];
...
int	pinky_best_move(t_mlx_data *data, int best_move, int actual_move)
{
	...
	if (actual_move == UP)
		result = abs((data->enemy.pinky.y - 1) - target_y) + abs(data->enemy.pinky.x - target_x);
	else if (actual_move == DOWN)
		result = abs((data->enemy.pinky.y + 1) - target_y) + abs(data->enemy.pinky.x - target_x);
	else if (actual_move == LEFT)
		result = abs(data->enemy.pinky.y - target_y) + abs((data->enemy.pinky.x - 1) - target_x);
